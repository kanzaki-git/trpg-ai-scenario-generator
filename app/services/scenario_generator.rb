  ...
  def prompt
    <<~PROMPT
      ...
      ・story_outlineには、ゲームマスターが
        シナリオ全体を理解するための解説を記載してください

      ・次の5つの見出しを、この順番で必ず使用してください
      ...
      ・事件の真相には、事件の原因、関係者、
        動機、最終的に明らかになる事実を記載してください

      ・出来事の順序には、事件が起きる前から
        クライマックスまでの出来事を
        時系列に沿って記載してください

      ・NPCの目的と行動理由には、主要なNPCごとに、
        何を目的として、なぜその行動を取ったかを記載してください

      ・手がかりが示していたことには、
        生成した各手がかりが何を意味し、
        どの真相につながるかを記載してください

      ・プレイヤーの判断とエンディングの関係には、
        クライマックスでの判断や結果によって、
        どのエンディングへつながるかを記載してください

      ・truth、NPC、clues、events、scenes、endingsに
        実際に生成した内容だけを整理して説明してください
      ...
      ・endingsには、クライマックスまでの
        プレイヤーの行動や判断によって分岐する
        2〜3個のエンディングを設定してください

      ・各エンディングのconditionには、
        ゲームマスターがどのエンディングを選ぶか判断できるように、
        クライマックスまでに確定した行動、判断、成否を
        具体的に記載してください

      ・conditionには、エンディングに入ってから
        新しく行う行動や判定を設定しないでください

      ・各エンディングのcontentは、
        クライマックスで重要な判断や判定の結果が
        確定した直後から始めてください

      ・プレイヤーの判断によって何が変わったのか、
        その場にどのような結果が起きたのかを
        原因と結果が分かるように描写してください

      ・各contentは、対応するconditionに記載した
      ...
      ・結果の要約だけで終わらせず、
        周囲の光景、音、空気の変化、
        主要なNPCの表情、言葉、行動などを含めてください

      ・プレイヤーたちが行ったことの意味や、
        その選択が登場人物や世界に与えた影響を描写してください

      ・事件の決着に続けて、主要なNPCや舞台が
        その後どうなったかを簡潔に描写してください

      ・最後の1〜2文では、舞台に残る光景や音、
        日常の変化などを描き、物語が終わったことが
        感じられる余韻を持たせてください

      ・失敗エンディングを含むすべてのcontentで、
        今回のセッションにおける結果を確定させてください

      ・すぐに新しい判断や対応が必要な緊急状態を残さず、
        その後の状況を描いて物語に区切りを付けてください

      ・シナリオに指定されたtoneと、
        そのエンディングの成否に合う雰囲気にしてください

      ・【完全成功】【失敗】などの見出し、
        箇条書き、結果を説明するだけの文章にはせず、
        一続きの物語として記載してください

      ・contentは、ゲームマスターがプレイヤーへ
        そのまま読み上げられる文章だけを記載してください

      ・contentには、ゲームマスターへの指示、
        行動選択肢、探索案内、判定の要求、
        プレイヤーへの質問を含めないでください
      ...
      ・結末を決める重要な判断や判定は
        最後のクライマックスシーンで行い、
        エンディングでは、その結果だけを描写してください

      ・シナリオ本編、truth、NPC、clues、eventsにない
        新しい真相や設定を追加しないでください

      ・各contentは、6〜10文、500文字以内を目安にしてください
      ...
      ・scenesには、導入からクライマックスまでの
        プレイヤーが行動する場面だけを設定してください
      ...
      ・最後のシーン以外では、シーンの終了時に
        プレイヤーが次に行うことを理解できるようにしてください

      ・最後のシーンでは、プレイヤーの判断や判定の結果が
        確定したところで終了し、結末そのものは描写しないでください
      ...
      ・titleには、何が起こるイベントか分かる
        簡潔なイベント名を設定してください

      ・trigger_conditionには、ゲームマスターが
        イベントを発生させる条件またはタイミングを記載してください

      ・trigger_conditionは、
        「保管庫端末の記録を確認した後」
        「警告音が3回鳴ったとき」のように、
        プレイヤーの行動や状況から判断できる具体的な条件にしてください

      ・read_aloud_textには、イベント発生時に
        ゲームマスターがプレイヤーへそのまま読み上げる文章を記載してください

      ・read_aloud_textには、その時点でプレイヤーが
        見る、聞く、感じることだけを含めてください

      ・イベントの原因、未公開の情報、
        ゲームマスター向けの指示はread_aloud_textに含めないでください

      ・gm_actionsには、読み上げ後にゲームマスターが行う説明、
        提示する選択肢、判定の案内などを記載してください

      ・post_event_changesには、イベント発生後に変化する状況と、
        その変化がいつまで続くかを記載してください

      ・イベントが発生する理由や物語への影響は、
        gm_actionsまたはpost_event_changesに含めてください

      ・同じ情報をtitle、trigger_condition、read_aloud_text、
        gm_actions、post_event_changesで繰り返さないでください

      ・複数のイベントを同じシーンに関連付ける場合も、
      ...
      ・最後のクライマックスシーンのtransition_conditionには、
        重要な判断や判定の結果が確定したら、
        その結果に合うエンディングを選ぶことを記載してください

      ・エンディングはscenesには含まれないため、
        エンディングのpositionを
        destination_scene_positionに設定しないでください
